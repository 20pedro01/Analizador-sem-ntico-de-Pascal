...
class SemanticAnalyzer {
    ...
    private function analyzeProgram(array $node): void {
        ...
        if (isset($node['const_section'])) {
            $this->analyzeConstSection($node['const_section']);
        }
        ...
        if (isset($node['var_section'])) {
            $this->analyzeVarSection($node['var_section']);
        }
    ...
    private function analyzeConstSection(array $node): void {
        ...
        foreach ($node['declarations'] as $decl) {
            ...
            $this->symbolTable->declare($decl['name'], $valueType, $decl['line'], 'constante');
        ...
        if (isset($node['params'])) {
            foreach ($node['params'] as $param) {
                $this->symbolTable->declare(
                    $param['name'], $param['type'], $param['line'] ?? $line, 'variable'
                );
                $this->symbolTable->markInitialized($param['name']);
                $this->symbolTable->incrementUse($param['name']);
            }
        }

        // Registrar el nombre de la función como variable (para Factorial := valor)
        if ($node['type'] === 'FunctionDecl') {
            $this->symbolTable->declare($name, $returnType, $line, 'variable');
            $this->symbolTable->markInitialized($name);
            $this->symbolTable->incrementUse($name);
        }

        // Procesar variables locales
        if (isset($node['var_section']) && $node['var_section']) {
            $this->analyzeVarSection($node['var_section']);
        }
    ...
    private function analyzeArrayAccess(array $node): string {
        ...
        // Analizar la expresión del índice
        if (isset($node['index'])) {
    ...
    private function analyzeIdentifier(array $node): string {
        ...
        if (!$symbol->initialized) {
            $this->errorHandler->addError(
                "Variable '$name' utilizada sin haber sido inicializada. " .
    ...
    private function checkUnusedVariables(): void {
        ...
        foreach ($unused as $symbol) {
            $this->errorHandler->addError(
                "Variable '{$symbol->name}' declarada en la línea {$symbol->line} pero nunca utilizada. " .
                "Considere eliminarla o verificar si debería estar siendo usada",
                $symbol->line,
                'semántico',
                'warning'
            );
